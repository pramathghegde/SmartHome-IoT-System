...
static bool relayPinForDevice(uint8_t deviceID, uint8_t& pin)
{
    switch (deviceID)
    {
        ...
        case OUTSIDE_BULB_DEVICE:
            pin = OUTSIDE_BULB_RELAY_PIN;
            return true;
...
void initRelays()
{
    ...
    digitalWrite(TUBE1_RELAY_PIN, HIGH);
    digitalWrite(TUBE2_RELAY_PIN, HIGH);
    digitalWrite(FAN_RELAY_PIN, HIGH);
    digitalWrite(EBIKE_RELAY_PIN, HIGH);
    digitalWrite(SOCKET_RELAY_PIN, HIGH);
    digitalWrite(OUTSIDE_BULB_RELAY_PIN, HIGH);
    digitalWrite(EXTRA1_RELAY_PIN, HIGH);
    digitalWrite(EXTRA2_RELAY_PIN, HIGH);

    pinMode(TUBE1_RELAY_PIN, OUTPUT);
    pinMode(TUBE2_RELAY_PIN, OUTPUT);
    pinMode(FAN_RELAY_PIN, OUTPUT);
    pinMode(EBIKE_RELAY_PIN, OUTPUT);
    pinMode(SOCKET_RELAY_PIN, OUTPUT);
    pinMode(OUTSIDE_BULB_RELAY_PIN, OUTPUT);
    pinMode(EXTRA1_RELAY_PIN, OUTPUT);
    pinMode(EXTRA2_RELAY_PIN, OUTPUT);

    writeRelay(TUBE1_RELAY_PIN, HIGH, "BOOT", "initRelays");
    writeRelay(TUBE2_RELAY_PIN, HIGH, "BOOT", "initRelays");
    writeRelay(FAN_RELAY_PIN, HIGH, "BOOT", "initRelays");
    writeRelay(EBIKE_RELAY_PIN, HIGH, "BOOT", "initRelays");
    writeRelay(SOCKET_RELAY_PIN, HIGH, "BOOT", "initRelays");
    writeRelay(OUTSIDE_BULB_RELAY_PIN, HIGH, "BOOT", "initRelays");
